      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div key={openCount} className="flex-1 overflow-y-auto px-6 py-4">
            {loading && <p className="text-sm text-neutral-500">Cargando ranking...</p>}
            {error && <p className="text-sm text-red-600">{error}</p>}
            {!loading && !error && renderedEntries.length === 0 && (
              <p className="text-sm text-neutral-500">Aún no hay parejas encontradas.</p>
            )}
              ...
              {renderedEntries.map((entry, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-base font-semibold text-neutral-900">{entry.coupleName}</p>
                    {entry.imageUrl && isRevealed && (
                      <div className="mt-3 opacity-0 animate-[fade-in_450ms_ease-out_forwards]">
                        <img
                          src={entry.imageUrl}
